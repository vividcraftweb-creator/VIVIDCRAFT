...
export function ArtworkCardComponent({ artwork, artistName: artistNameProp, onDelete }: ArtworkCardProps) {
  ...
  return (
    <>
      <div className="group relative rounded-xl sm:rounded-2xl overflow-hidden bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 hover:border-amber-400/80 dark:hover:border-amber-500/50 transition-all duration-300 hover:-translate-y-0.5 shadow-sm hover:shadow-md dark:hover:shadow-amber-500/10 flex flex-col self-start w-full">
        ...
        >
          ...
          {/* Top-Left Pricing Status Badge (Compact: text-[9px] px-1.5 py-0.5) */}
          ...
          <div className="absolute bottom-1.5 right-1.5 z-30 p-1 rounded-md bg-black/50 backdrop-blur-md text-white/80 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/70 hover:text-white shadow-md">
          ...
          <div className="flex items-center gap-1">
            {canDelete && (
              <button
                ...
                title="Delete Post"
                className="inline-flex items-center justify-center p-0.5 rounded text-xs text-rose-500 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors cursor-pointer"
              >
          ...
          <div className="bg-slate-50/70 dark:bg-slate-900/90 border-t border-slate-100 dark:border-slate-800/80 p-2 sm:p-3 flex flex-col gap-2 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              {showAvatarImage ? (
                ...
                <img
                  src={avatarUrl!}
                  alt={displayName}
                  className="w-5 h-5 sm:w-6 sm:h-6 rounded-full object-cover shrink-0 border border-slate-200 dark:border-slate-700 shadow-sm"
                  onError={() => setAvatarError(true)}
                />
              ) : (
                <div
                  className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-[#A2694E]/15 border border-[#A2694E]/30 flex items-center justify-center text-[10px] font-bold text-[#A2694E] dark:text-[#C58B6F] shrink-0 select-none"
            ...
            <div className="pt-1.5 border-t border-slate-200/60 dark:border-slate-800/60 flex flex-wrap items-center justify-between gap-1.5">
              ...
              <div className="flex items-center gap-1">
                ...
                >
                  <ZoomIn className="h-2.5 w-2.5 sm:h-3 sm:w-3 text-amber-600 dark:text-amber-400" />
                  <span>Details</span>
                </button>
              </div>
            </div>
