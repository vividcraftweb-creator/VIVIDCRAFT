...
export function FeaturedCrew() {
  const [featuredMember, setFeaturedMember] = useState<CrewMember>(FALLBACK_CREW[0]);
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        // 1. Fetch featured crew
        const { data: crewData } = await supabase
          .from('crew_members')
          .select('*')
          .order('display_order', { ascending: true });

        if (Array.isArray(crewData) && crewData.length > 0 && isMounted) {
          const featured = crewData.find((c: CrewMember) => c.is_featured) || crewData[0];
          if (featured) setFeaturedMember(featured);
        } else {
          const resCrew = await fetch('/api/crew');
          if (resCrew.ok) {
            const json = await resCrew.json();
        ...
        if (Array.isArray(revData) && revData.length > 0 && isMounted) {
          setReviews(revData);
        } else {
          const resRev = await fetch('/api/reviews');
          if (resRev.ok) {
            const json = await resRev.json();
            if (json?.reviews && Array.isArray(json.reviews) && isMounted) {
              setReviews(json.reviews);
            }
          }
        }
      } catch (err) {
        console.warn('Failed to load featured crew or reviews:', err);
      }
    }
