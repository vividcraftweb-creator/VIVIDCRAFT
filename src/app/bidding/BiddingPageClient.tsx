...
function formatTimeRemaining(endTimeStr: string | null | undefined): string {
...
export default function BiddingPageClient() {
  ...
  const getWhatsAppBidUrl = (art: RankedArtwork) => {
    ...
    const currentBidVal = Number(art.current_bid || art.starting_bid || art.price || 0);
    const bidDisplay = currentBidVal > 0 ? `LKR ${currentBidVal.toLocaleString()}` : 'Open';

    const fullMessage = `Hi, I want to place a bid on "${title}" (Ref ID: #${refId}) by ${artistName}. Current Bid: ${bidDisplay}.`;
              ...
              >
                ...
                >
                  ...
                  <div className="absolute top-3 inset-x-3 flex items-center justify-between pointer-events-none">
                    {/* Open Bidding vs Concluded Badge */}
                    {artwork.status === 'ENDED' ? (
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-slate-800/90 text-slate-300 border border-slate-700 shadow-lg backdrop-blur-sm">
                  ...
                  <div className="absolute bottom-3 left-3 pointer-events-none flex flex-col gap-1 items-start">
                    <div className="px-3 py-1.5 rounded-lg bg-black/70 backdrop-blur-md border border-amber-500/40 text-left">
                      <span className="text-[10px] uppercase font-bold tracking-wider text-amber-400 block leading-tight">
                        {artwork.current_bid && artwork.current_bid > (artwork.starting_bid || 0) ? 'Current Bid' : 'Starting Bid'}
                      </span>
                      <span className="text-sm font-extrabold text-white">
                        LKR {Number(artwork.current_bid || artwork.starting_bid || 0).toLocaleString()}
                      </span>
                    </div>
                    {artwork.end_time && (
                      <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-black/70 border border-slate-700 text-[10px] font-medium text-slate-200 backdrop-blur-md">
                        <Clock className="w-2.5 h-2.5 text-amber-400" />
                        <span>{formatTimeRemaining(artwork.end_time)}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  ...
                  <div className="mt-4 pt-3">
                    {artwork.status === 'ENDED' ? (
                      <button
                        disabled
                        className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold bg-slate-800 text-slate-400 border border-slate-700 cursor-not-allowed"
                      >
                        <span>Auction Concluded</span>
                      </button>
                    ) : (
            ...
            <div className="md:w-2/5 p-6 sm:p-8 flex flex-col justify-between overflow-y-auto bg-white dark:bg-[#111827]">
              <div>
                ...
                <div className="my-5 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200/80 dark:border-amber-500/25">
                  <div className="flex items-center justify-between">
                    <span className="text-xs uppercase font-bold tracking-wider text-amber-700 dark:text-amber-300 block">
                      {selectedArtwork.current_bid && selectedArtwork.current_bid > (selectedArtwork.starting_bid || 0) ? 'Current Highest Bid' : 'Starting Bid Amount'}
                    </span>
                    {selectedArtwork.status === 'ENDED' ? (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-700 text-slate-300">Concluded</span>
                    ) : (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/20 text-amber-400 border border-amber-500/30">Live</span>
                    )}
                  </div>
                  <div className="text-2xl font-extrabold text-slate-900 dark:text-white mt-1">
                    LKR {Number(selectedArtwork.current_bid || selectedArtwork.starting_bid || 0).toLocaleString()}
                  </div>
                  {selectedArtwork.starting_bid && selectedArtwork.current_bid && selectedArtwork.current_bid > selectedArtwork.starting_bid && (
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">
                      Starting bid was: LKR {Number(selectedArtwork.starting_bid).toLocaleString()}
                    </p>
                  )}
                  {selectedArtwork.end_time && (
                    <p className="text-xs font-medium text-amber-600 dark:text-amber-400 mt-1.5 flex items-center gap-1">
                      <Clock className="w-3.5 h-3.5" />
                      <span>{selectedArtwork.status === 'ENDED' ? 'Ended' : `Ends in: ${formatTimeRemaining(selectedArtwork.end_time)}`}</span>
                    </p>
                  )}
              ...
              <div className="pt-6 flex flex-col gap-3">
                {selectedArtwork.status === 'ENDED' ? (
                  <button
                    disabled
                    className="w-full inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-bold text-sm bg-slate-800 text-slate-400 border border-slate-700 cursor-not-allowed"
                  >
                    <span>Auction Has Concluded</span>
                  </button>
                ) : (
                  <a
