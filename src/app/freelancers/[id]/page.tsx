  ...
  if (profile) {
    ...
    try {
      const results = await Promise.all([
        ...
        supabase.from('reviews').select('*').eq('artist_id', pId).order('created_at', { ascending: false }),
      ]);
      ed = results[0]?.data || [];
      ex = results[1]?.data || [];
      po = results[2]?.data || [];
      ...
      let rawReviews = results[6]?.data || [];

      // Fallback if reviews table had 0 rows but artist_reviews has rows
        ...
        if (clientIds.length > 0) {
          ...
          if (clientProfiles) {
            clientProfiles.forEach((p: any) => {
              const name = p.full_name || `${p.first_name || ''} ${p.last_name || ''}`.trim() || 'Verified Client';
      ...
      if (rawArtworks.length > 0) {
        ...
        initialArtworks = rawArtworks.map((art: any) => {
          ...
          return {
            ...
            artist: {
              id: pId,
              name: (profile.artist_name || '').trim() || (profile.full_name || '').trim() || `${fName} ${lName}`.trim() || 'Verified Artist',
              avatar_url: picVal || null,
              title: titleVal || 'Verified Artist',
              role: profile.role || 'artist',
            },
          };
        });
    ...
    const initialProfile: any = {
      ...
      art_styles: profile.art_styles || profile.mediums || [],
      art_specialties: profile.art_specialties || profile.specialties || [],
      services_offered: profile.services_offered || profile.services || [],
      mediums: profile.mediums || profile.art_styles || [],
      specialties: profile.specialties || profile.art_specialties || [],
      services: profile.services || profile.services_offered || [],
